# /data03/hjj/hjj/LFS-new/newwork/region/02_plot_sst_table.py
...
# 🎯 自动匹配文件名
INPUT_CSV = os.path.join(DATA_OUT_DIR, 'SST_Evaluation_20230101-20241231.csv')
...
def draw(data):
    ...
    for i, row in enumerate(reversed(data)):
        ...
        for j, v in enumerate(row[1:]):
            ...
            ax.text(x_c[j+1], y, txt, ha='center', va='center', size=12)
    ...
    for k, t in enumerate(['Bias', 'MAE', 'RMSE']):
        ax.text((x_pos[1+k*3]+x_pos[4+k*3])/2, y2, f"{t} (℃)", ha='center', va='center', weight='bold', size=14)
    ...
    plt.title('Sea Surface Temperature (SST) Evaluation', pad=25, size=16, weight='bold')
    plt.savefig(os.path.join(PLOT_OUT_DIR, 'Table_SST_Combined.png'), bbox_inches='tight')
